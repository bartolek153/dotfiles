...
local function restore_session(prompt_bufnr, _)
  actions.select_default:replace(function()
    ...
    vim.cmd(cmd)
    vim.notify(session_name, "Session restored")
  end)
  return true
end
...
local function delete_session(prompt_bufnr, _)
  actions.select_default:replace(function()
    ...
    if vim.fn.delete(session_name) == 0 then
      vim.notify(session_name, "Session deleted")
    end
  end)
  return true
end
...
function M.save_session()
  vim.ui.input({ prompt = "Input session name: ", default = default_session_name }, function(session_name)
    if session_name then
      ...
      vim.notify(session_name, "Session saved")
    end
  end)
end
...
function M.toggle_session()
  if track_session then
    ...
    track_session = false
    vim.notify("Session tracking disabled", "Session")
  else
    vim.ui.input({ prompt = "Input session name: ", default = default_session_name }, function(session_name)
      if session_name then
        session_name = vim.g.session_dir .. "/" .. session_name
        ...
        track_session = true
        vim.notify("Session tracking enabled", "Session")
      end
    end)
  end
end
